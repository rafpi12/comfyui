...
# One worker process for heavy inference — keeps event loop free
...
async def download_and_load_models():
    ...
    # Load models inside the worker process so they live there permanently
...
# Everything below runs INSIDE the worker process (separate memory space).
# The worker caches the models after first load so subsequent files are fast.
...
def _worker_load_models():
    """Called once in the worker process to load models into GPU memory."""
...
def _worker_analyze(audio_path: str) -> dict:
    ...
    MP = [6.35,2.23,3.48,2.33,4.38,4.09,2.52,5.19,2.39,3.66,2.29,2.88]
    mp = [6.33,2.68,3.52,5.38,2.60,3.53,2.54,4.75,3.98,2.69,3.34,3.17]
    KN = ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B']
    MP, mp = map(np.array, (MP, mp))
...
def _worker_process_file(audio_path: str, output_dir: str) -> dict:
    """
    Runs entirely in the worker process.
    Returns {"ok": True, "caption": "..."} or {"ok": False, "error": "..."}
    """
    import soundfile as sf, librosa as lb, traceback as tb
    try:
        _worker_load_models()  # no-op if already loaded
...
# ── CAPTIONING ORCHESTRATOR (runs in event loop, calls worker) ────────────────
...
async def run_captioning():
    ...
    log(f"🎵 Démarrage — {len(wav_paths)} fichier(s) à traiter → {output_dir}", "success")
    ...
    for i, audio_path in enumerate(wav_paths):
        ...
        # Submit to worker process — event loop is FREE during inference
        result = await loop.run_in_executor(
            process_pool, _worker_process_file, audio_path, output_dir
        )

        if result["ok"]:
            log(f"   ✅ BPM:{result['bpm']} | {result['keyscale']} | {result['duration']}s", "success")
...
async def start_captioning(request: Request):
    ...
    log(f"🚀 Batch reçu — {len(wav_paths)} fichier(s)", "success")
